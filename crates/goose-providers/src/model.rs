...
use crate::config::{
    default_provider_config_store, ProviderConfigError, ProviderConfigExt, ProviderConfigStore,
};
...
fn get_config_param<T: serde::de::DeserializeOwned>(
    config: &dyn ProviderConfigStore,
    key: &str,
) -> Result<T, ProviderConfigError> {
    config.get_param(key)
}
...
impl ModelConfig {
    pub fn new(model_name: &str) -> Result<Self, ConfigError> {
        let config_store = default_provider_config_store();
        Self::new_with_config(model_name, config_store.as_ref())
    }

    pub fn new_with_config(
        model_name: &str,
        config_store: &dyn ProviderConfigStore,
    ) -> Result<Self, ConfigError> {
        Self::new_base(model_name.to_string(), None, config_store)
    }
    ...
    pub fn new_with_context_config(
        ...
        let config = Self::new_base(model_name, context_env_var, config_store)?;
    ...
    fn new_base(
        ...
        let context_limit = if let Some(env_var) = context_env_var {
            match get_config_param::<usize>(config_store, env_var) {
                Ok(limit) => Some(Self::validate_context_limit(limit, env_var)?),
            ...
            match get_config_param::<usize>(config_store, "GOOSE_CONTEXT_LIMIT") {
                Ok(limit) => {
                    if limit == 0 {
        ...
        let max_tokens = Self::parse_max_tokens(config_store)?;
        let temperature = Self::parse_temperature(config_store)?;
        let toolshim = Self::parse_toolshim(config_store)?;
        let toolshim_model = Self::parse_toolshim_model(config_store)?;
    ...
    fn validate_context_limit(limit: usize, env_var: &str) -> Result<usize, ConfigError> {
        if limit < 4 * 1024 {
            return Err(ConfigError::InvalidRange(
                env_var.to_string(),
    ...
    fn parse_temperature(
        ...
        match get_config_param::<f32>(config_store, "GOOSE_TEMPERATURE") {
            Ok(temp) => {
                if temp < 0.0 {
                    return Err(ConfigError::InvalidRange(
    ...
    fn parse_max_tokens(
        ...
        match get_config_param::<i32>(config_store, "GOOSE_MAX_TOKENS") {
            Ok(tokens) => {
                if tokens <= 0 {
                    return Err(ConfigError::InvalidRange(
    ...
    fn parse_toolshim(config_store: &dyn ProviderConfigStore) -> Result<bool, ConfigError> {
        match get_config_param::<bool>(config_store, "GOOSE_TOOLSHIM") {
            Ok(enabled) => Ok(enabled),
            Err(ProviderConfigError::NotFound(_)) => Ok(false),
            Err(e) => Err(ConfigError::InvalidValue(
    ...
    fn parse_toolshim_model(
        config_store: &dyn ProviderConfigStore,
    ) -> Result<Option<String>, ConfigError> {
        match get_config_param::<String>(config_store, "GOOSE_TOOLSHIM_OLLAMA_MODEL") {
    ...
    fn legacy_thinking_effort() -> Option<ThinkingEffort> {
        ...
        if let Ok(value) = get_config_param::<String>(config_store.as_ref(), "CLAUDE_THINKING_TYPE")
        {
        ...
        if let Ok(enabled) =
            get_config_param::<bool>(config_store.as_ref(), "CLAUDE_THINKING_ENABLED")
        {
        ...
        if let Ok(value) =
            get_config_param::<String>(config_store.as_ref(), "GEMINI3_THINKING_LEVEL")
        {
    ...
    pub fn get_config_param<T: for<'de> serde::Deserialize<'de>>(
        ...
        self.request_params
            ...
            .or_else(|| {
                let config_store = default_provider_config_store();
                get_config_param::<T>(config_store.as_ref(), config_key).ok()
            })
    }
...
fn is_openai_responses_model(model_name: &str) -> bool {
    static RE: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"(?i)(?:^|[-/])(?:o\d+(?:$|-)|gpt-5(?:$|[-.]))").unwrap());
    RE.is_match(model_name)
}

fn extract_reasoning_effort(model_name: &str) -> (String, Option<String>) {
...
mod tests {
    ...
    fn test_parse_max_tokens_valid() {
        ...
        let result =
            ModelConfig::parse_max_tokens(default_provider_config_store().as_ref()).unwrap();
    ...
    fn test_parse_max_tokens_invalid_string() {
        ...
        let result = ModelConfig::parse_max_tokens(default_provider_config_store().as_ref());
