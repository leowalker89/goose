...
#[cfg(test)]
mod tests {
    use super::{Message, MessageContent, MessageMetadata};
    use rmcp::model::{
        AnnotateAble, CallToolRequestParams, JsonObject, PromptMessage, PromptMessageContent,
        ...
        let message = Message::assistant()
            ...
            .with_tool_request(
                ...
                Ok(CallToolRequestParams::new("test_tool")
                    .with_arguments(json_object(serde_json::json!({"param": "value"})))),
            );
    ...
    fn test_deserialization() {
        ...
        if let MessageContent::ToolRequest(req) = &message.content[1] {
            ...
            if let Ok(tool_call) = &req.tool_call {
                ...
                assert_eq!(
                    tool_call.arguments,
                    Some(json_object(serde_json::json!({"param": "value"})))
                )
            } else {
    ...
    fn test_message_with_tool_request() {
        let tool_call = Ok(CallToolRequestParams::new("test_tool")
