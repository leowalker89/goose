...
//!    [`crate::agents::headroom::signals::LineImportanceDetector`] priority signals +
//!    config-supplied keywords).
//! 3. Sort files by total match score; cap to `max_files`.
//! 4. Run [`crate::agents::headroom::transforms::adaptive_sizer::compute_optimal_k`] over
...
use crate::agents::headroom::ccr::CcrStore;
use crate::agents::headroom::signals::{ImportanceContext, LineImportanceDetector};
use crate::agents::headroom::transforms::adaptive_sizer::compute_optimal_k;
...
/// to a [`crate::agents::headroom::signals::KeywordDetector`].
...
impl SearchCompressor {
    pub fn new(config: SearchCompressorConfig) -> Self {
        Self {
            config,
            importance: Box::new(crate::agents::headroom::signals::KeywordDetector::new()),
        }
    }
                ...
                if self.config.boost_errors {
                    ...
                    if let Some(category) = signal.category {
                        ...
                        let bump = match category {
                            crate::agents::headroom::signals::ImportanceCategory::Error => 0.5,
                            crate::agents::headroom::signals::ImportanceCategory::Warning => 0.4,
                            crate::agents::headroom::signals::ImportanceCategory::Importance => 0.3,
                            // Categories below aren't part of
                            // PRIORITY_PATTERNS_SEARCH; preserve Python's
                            // behavior of not boosting for them.
                            crate::agents::headroom::signals::ImportanceCategory::Security
                            | crate::agents::headroom::signals::ImportanceCategory::Markdown => 0.0,
                        };
                        score += bump;
                    }
                }
...
mod tests {
    use super::*;
    use crate::agents::headroom::ccr::InMemoryCcrStore;
